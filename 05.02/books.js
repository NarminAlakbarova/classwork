...
function drawRow(array) {
  array.forEach((item) => {
    ...
    <button id="${item.id} class="btn">Add to favorites</button>
  </div>
    `;
    row.append(col);
  });
...
bookLocaleS = JSON.parse(localStorage.getItem("bookLocaleS")) || [];
btn.forEach((item) => {
  item.addEventListener("click", function () {
    let booksFav = books.find((element) => element.id === +item.id);
    bookLocaleS.push(booksFav);
  });
  localStorage.setItem("booksFav", JSON.stringify(bookLocaleS));
});
