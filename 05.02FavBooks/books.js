...
let allBtn = document.querySelectorAll(".btn-primary");
allBtn.forEach((item) => {
  item.addEventListener("click", function () {
    let selectedObj = books.find((obj) => obj.id == item.id);
    allBooks.includes(selectedObj);
    if (!allBooks.includes(selectedObj)) {
      let favBooks = books.find((obj) => obj.id == item.id);
      allBooks.push(favBooks);
      localStorage.setItem("favBooks", JSON.stringify(allBooks));
    } else {
